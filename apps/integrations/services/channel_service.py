...
from apps.integrations.services.channel_credentials import encrypt_value
...
def create_channel_account(tenant: Tenant, form: ChannelAccountForm) -> ChannelAccount:
    ...
    _upsert_credential(tenant, account, data)
    return account


def update_channel_account(account: ChannelAccount, form: ChannelAccountForm) -> ChannelAccount:
    data = form.cleaned_data
    ...
    _upsert_credential(account.tenant, account, data)
    return account


def _upsert_credential(tenant: Tenant, account: ChannelAccount, data: dict) -> ChannelCredential:
    cred, _ = ChannelCredential.objects.get_or_create(
        tenant=tenant,
